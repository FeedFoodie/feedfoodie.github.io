...
// --- UPDATED: Invisible Watermark using your CSS classes ---
function addInvisibleWatermark(text) {
  // Generate unique ID
  const watermarkId = Date.now().toString(36) + Math.random().toString(36).substr(2, 5);
  
  // Create watermark using your invisible CSS classes
  // Use foodiie or foodiee which are already invisible in your CSS
  const watermark = `\n\n<!-- foodiie -->NorthBladeTL:${watermarkId}\n\n`;
  
  // Add at the beginning of the text (before TL line)
      ...
      try {
        ...
        if (contentType.includes("text/markdown") || contentType.includes("text/plain")) {
          ...
          if (path.startsWith('/SIMB') || path.startsWith('/ABSW') || path.startsWith('/LNB') || path.startsWith('/HERO')) {
            // First add invisible watermark at the beginning
            const textWithWatermark = addInvisibleWatermark(originalText);
            // Then add the TL line
